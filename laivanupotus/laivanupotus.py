...
muutokset:
toteutettu tietorakenne paremmin
...
class tekoaly_ampuminen_tiedot:
    def __init__(self):
        self.vaaka = False #onko laiva vaaka asennossa
        self.pysty = False #onko laiva pysty asennossa
        self.upottaminen = False  #onko laivan upottaminen kesken
        self.oikealle = True#ammutaan oikealle puolelle False = vasemmalle
        self.ylos = True #ammutaan ylös False = alas
        #tämä sisältää tiedon mihin osutun kohdan viereen ammutaan
        self.luku = 1 #1 = ylös 2 = oikea 3 = alas

        self.sijainti_vaaka = 0
        self.sijainti_pysty = 0


#käytetään edellistä tietorakenteita
tiedot = tekoaly_ampuminen_tiedot() 


def tekoaly_ampuminen(pelaajan_laivat,tiedot):
    ...
    while True:
        if tiedot.upottaminen == False: #jos minkää laivan upottaminen ei ole kesken
            tiedot.sijainti_pysty =  random.randint(0,9) #arvotaan kohta
            tiedot.sijainti_vaaka = random.randint(0,9)
            if lista_tarkistus_tekoaly(pelaajan_laivat,tiedot.sijainti_pysty,tiedot.sijainti_vaaka) == False: #tarkistaa onko kohdan vieressä upotettu laiva
                continue #jos on ei ammuta siihen

            osu,osuttu,pelaajan_laivat = laivat_tarkistus(pelaajan_laivat,tiedot.sijainti_pysty,tiedot.sijainti_vaaka) #funktio joka tarkistaa mitä ammutussa kohdassa on
        ...
        if tiedot.upottaminen: #jos laivan upottaminen kesken
            ...
            pelaajan_laivat,upposi = uponnut_tarkistus(pelaajan_laivat, tiedot.sijainti_pysty, tiedot.sijainti_vaaka) #tutkii onko laiva uponnut
            ...
            if tiedot.vaaka: #vaaka-asennossa
                if tiedot.oikealle: #oikealle
                    if tiedot.sijainti_vaaka +1 <= 9: #tutkitaan loppuuko kartta
                        tiedot.sijainti_vaaka += 1
                        if pelaajan_laivat[tiedot.sijainti_pysty][tiedot.sijainti_vaaka] == 3: #tarkistetaan onko kohtaan ammuttu ja tyhjä
                            tiedot.oikealle = False #vasemmalle
                        elif pelaajan_laivat[tiedot.sijainti_pysty][tiedot.sijainti_vaaka] != 2:
                            osu,osuttu,pelaajan_laivat = laivat_tarkistus(pelaajan_laivat,tiedot.sijainti_pysty,tiedot.sijainti_vaaka) #ammutaan kohtaan
                ...
                if tiedot.oikealle == False: #vasemmalle
                    tiedot.sijainti_vaaka -= 1
                    if pelaajan_laivat[tiedot.sijainti_pysty][tiedot.sijainti_vaaka] != 2:
                        osu,osuttu,pelaajan_laivat = laivat_tarkistus(pelaajan_laivat,tiedot.sijainti_pysty,tiedot.sijainti_vaaka) #ammutaan kohtaan
            ...
            ###########################################
                ...
                if tiedot.ylos: #ylös
                    if tiedot.sijainti_pysty -1 >= 0: #tutkitaan loppuuko kartta
                        tiedot.sijainti_pysty -= 1
                        if pelaajan_laivat[tiedot.sijainti_pysty][tiedot.sijainti_vaaka] == 3: #tarkistetaan onko kohtaan ammuttu ja tyhjä
                            tiedot.ylos = False #alas
                        elif pelaajan_laivat[tiedot.sijainti_pysty][tiedot.sijainti_vaaka] != 2:
                            osu,osuttu,pelaajan_laivat = laivat_tarkistus(pelaajan_laivat,tiedot.sijainti_pysty,tiedot.sijainti_vaaka)
                            if osu == False: #jos ei osuttu
                                tiedot.ylos = False #alas
                                break
                    else: #jos kartta loppuu
            ...
            #jos ei tutkitaan mitä osutun kohdan ympärillä on
                ...
                if tiedot.sijainti_pysty -1 < 0: #jos kartta loppuu
                    tiedot.luku = 2
                elif pelaajan_laivat[tiedot.sijainti_pysty -1][tiedot.sijainti_vaaka] == 1 or pelaajan_laivat[tiedot.sijainti_pysty -1][tiedot.sijainti_vaaka] == 4: #jos kohdan yläpuolella ampumaton kohta ammutaan siihen
                    osu,osuttu,pelaajan_laivat = laivat_tarkistus(pelaajan_laivat, tiedot.sijainti_pysty -1,tiedot.sijainti_vaaka) #funktio joka tarkistaa mitä ammutussa kohdassa on
                    if osu: #jos osuttiin
                        tiedot.pysty = True
                    else: #jos ei osuttu
                        tiedot.luku = 2
                ...
                if tiedot.sijainti_vaaka +1 > 9: #jos kartta loppu
                    tiedot.luku = 3
                elif pelaajan_laivat[tiedot.sijainti_pysty][tiedot.sijainti_vaaka +1] == 1 or pelaajan_laivat[tiedot.sijainti_pysty][tiedot.sijainti_vaaka +1] == 4: #jos kohdan oikealla puolella ampumaton kohta ammutaan siihen
                    osu,osuttu,pelaajan_laivat = laivat_tarkistus(pelaajan_laivat,tiedot.sijainti_pysty,tiedot.sijainti_vaaka +1,) #funktio joka tarkistaa mitä ammutussa kohdassa on
                    if osu: #jos osuttiin
                        tiedot.vaaka = True
                    else: #jos ei osuttu
                        tiedot.luku = 3
                ...
                if tiedot.sijainti_pysty +1 > 9: #jos kartta loppu
                    tiedot.vaaka = True
                    tiedot.luku = 1
                elif pelaajan_laivat[tiedot.sijainti_pysty +1][tiedot.sijainti_vaaka] == 1 or pelaajan_laivat[tiedot.sijainti_pysty +1][tiedot.sijainti_vaaka] == 4: #jos kohdan alapuolella ampumaton kohta ammutaan siihen
                    osu,osuttu,pelaajan_laivat = laivat_tarkistus(pelaajan_laivat,tiedot.sijainti_pysty +1,tiedot.sijainti_vaaka) #funktio joka tarkistaa mitä ammutussa kohdassa on
                    if osu: #jos osuttiin
                        tiedot.pysty = True
                    else: #jos ei osuttu
...
while True: #pääsilmukka
    ...
    if pelaajan_vuoro == False:
        ...
        pygame.display.flip() #päivittää näytön
        #tekoälyn toiminnot
        pelaajan_laivat = tekoaly_ampuminen(pelaajan_laivat,tiedot) #funktio joka hoitaa sen mihin tekoäly ampuu
